...
- Confirmar RLS/autorização server-side quando houver recurso tenant-scoped. Não aplicável.
- Registar risco residual ou decisão temporária. Bug real encontrado no motor de disponibilidade (`generateTimezoneAwareSlots`/`NEX-061`): a primeira vaga de "hoje", quando decidida por `min_notice_hours` em vez do horário de abertura, não era arredondada à grelha de `slot_interval_minutes` — ficava ancorada ao milissegundo exato de `Date.now()`, o que quebrava a verificação de conflitos de recorrência (`NEX-121`) sempre que a primeira ocorrência de uma série caía em "hoje". **Corrigido** (pedido explícito do dono, mesma sessão): janela passa a ser ancorada em `interval.startMs` e arredondada para cima até à grelha, sem alterar nenhum dia já decidido pelo horário de abertura; coberto por 2 testes unitários novos e pelo próprio E2E de recorrência. Ver `docs/evidence/NEX-205_FIX_MOTOR_DISPONIBILIDADE.md` e `docs/10_RISK_REGISTER.md` (R13).
