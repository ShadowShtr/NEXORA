-- Visual refinement mid-2026 (paginated public flow): the "Resumo da marcação" screen
-- has an optional "Observações" field, same as the dona's own manual booking already
-- supports (create_manual_booking, 0009_create_manual_booking.sql) — appointments.
-- client_observation has existed since 0001_initial.sql but create_public_booking never
-- accepted it. Adds p_client_observation as a new trailing parameter with a default of
-- null, so the existing 8-argument call shape (booking-actions.ts, before this change)
-- keeps working unmodified — CREATE OR REPLACE can add a defaulted trailing parameter
-- without dropping the function first. Same nullif(..., '') empty-string-to-null
-- normalization as the manual-booking path, so an empty textarea submits as no
-- observation rather than an empty string.
create or replace function public.create_public_booking(
  ...
  p_client_observation text default null
)
...
begin
  ...
  insert into public.appointments
    (id, tenant_id, client_id, source, status, start_at, end_at, blocked_until,
     ...
     idempotency_key_hash, idempotency_payload_hash, client_observation)
  values
