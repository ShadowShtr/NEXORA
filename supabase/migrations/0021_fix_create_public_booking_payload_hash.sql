-- Fixes a real bug in 0018_booking_lookup_code.sql's create_public_booking: the payload
-- fingerprint tried to sort the selected service ids via a separate `order by x::text`
-- on the outer SELECT around string_agg(), which Postgres rejects (42803: "column x.x
-- must appear in the GROUP BY clause or be used in an aggregate function") — ORDER BY
-- on a scalar column can't sit next to an aggregate result with no GROUP BY unless it's
-- the aggregate's own internal ORDER BY. Confirmed live: every call to
-- create_public_booking with p_selected_service_ids failed with this error — the public
-- booking flow has never been able to complete a booking that includes an individually
-- selected service (only an empty-array/package-only booking could have avoided
-- unnest() and string_agg() firing at all). Fix: move ORDER BY inside string_agg()
-- (`string_agg(x::text, ',' order by x::text)`), the standard syntax for "aggregate a
-- sorted list" — same intent (equivalent requests with items in a different order still
-- hash identically), correct syntax. No other line changes from 0018's version.
