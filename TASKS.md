...
- [x] **NEX-178** — Corrigir concorrência do cliente em `/resumo` e adicionar E2E ao CI _(depende: NEX-065,NEX-175)_ — o bug do cliente React (ambas as páginas presas em "A confirmar…") foi corrigido em `fix/public-booking-concurrency` (contrato `PublicBookingResult` discriminado, `submitPublicBooking` isolado, máquina de estados, `AbortController`+`finally`), com um deadlock real do Postgres (`40P01`) descoberto e corrigido via retry no mesmo PR — validado 20/20 (2 concorrentes, incl. lote de 40 execuções capturando e confirmando o retry do deadlock), 10/10 (3 concorrentes), 5/5 (5 concorrentes) contra o Supabase de dev/preview real. Job `e2e-critical` adicionado a `.github/workflows/ci.yml` (build de produção + Supabase local + `playwright test --grep @critical`, 6 specs marcados: login, reserva pública, corrida de reserva, criação manual, conclusão/pagamento, isolamento entre tenants) — condicionado a `verify`+`integration` passarem primeiro; validado localmente 14/14. Ao marcar `appointment-completion.spec.ts` como `@critical` e rodá-lo pela primeira vez, revelou-se completamente desatualizado desde o redesign da agenda (`#59`-`#62`): seletor `.appointment-card` inexistente, e o fluxo assumia o formulário de conclusão inline no card em vez do `CompletionSheet` (bottom sheet) atual — reescrito e validado 5/5. Falta apenas tornar os 3 checks obrigatórios na proteção do branch `main` (configuração do GitHub, fora do alcance do Claude Code, ver `docs/evidence/NEX-178_PUBLIC_BOOKING_CLIENT_CONCURRENCY.md`)
