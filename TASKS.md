...
- [!] **NEX-178** — Corrigir concorrência do cliente em `/resumo` e adicionar E2E ao CI _(depende: NEX-065,NEX-175)_ — bug do cliente React e deadlock do Postgres já corrigidos e mesclados (#138, ver `docs/evidence/NEX-178_PUBLIC_BOOKING_CLIENT_CONCURRENCY.md`). Job `e2e-critical` implementado em `task/NEX-178-e2e-critical-ci` (PR aberto, por mesclar): `.github/workflows/ci.yml` ganhou o job (build de produção + `supabase start` + `playwright test --grep @critical`), 6 specs marcadas `@critical` (login, fluxo público completo, corrida, conclusão/pagamento, isolamento entre tenants, wizard de marcação manual), teste de corrida com `retries: 0` explícito. Ainda bloqueada até: (1) o job correr verde pela primeira vez no GitHub Actions — não validável nesta máquina, sem Docker (`ADR-007`); (2) a proteção de branch do `main` passar a exigir `e2e-critical` (configuração do GitHub, feita pela dona); (3) falta spec E2E completa de NEX-085 (criação manual de marcação) — só existe uma spec parcial (NEX-092) que para no passo de seleção de cliente
