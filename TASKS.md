...
- [!] **NEX-178** — Corrigir concorrência do cliente em `/resumo` e adicionar E2E ao CI _(depende: NEX-065,NEX-175)_ — bug do cliente React e deadlock do Postgres já corrigidos e mesclados (#138, ver `docs/evidence/NEX-178_PUBLIC_BOOKING_CLIENT_CONCURRENCY.md`). Job `e2e-critical` implementado em `task/NEX-178-e2e-critical-ci` (**PR #139, todos os checks verdes incl. `E2E crítico` a correr de facto contra `supabase start` no runner do GitHub — 3m47s, sem falhas — por mesclar pela dona**): `.github/workflows/ci.yml` ganhou o job (build de produção + `supabase start` + `playwright test --grep @critical`), 6 specs marcadas `@critical`. Ao validar, `appointment-completion.spec.ts` (NEX-110) estava genuinamente partido (não ambiente) — o redesign da agenda mudou `.appointment-card` para `.appointment-timeline-card` e moveu o formulário de conclusão para um bottom sheet separado (`CompletionSheet.tsx`), e a spec nunca foi atualizada porque E2E nunca corria em CI; corrigido nesta tarefa. Restam apenas: (1) a proteção de branch do `main` passar a exigir `e2e-critical`/`gitleaks`/`analyze` como checks obrigatórios (configuração do GitHub, feita pela dona, depois do merge); (2) falta spec E2E completa de NEX-085 (criação manual de marcação) — só existe uma spec parcial (NEX-092); (3) `appointment-completion-discount.spec.ts`, `appointment-completion-extras.spec.ts` e `appointment-card.spec.ts` têm o mesmo padrão desatualizado do NEX-110 mas não foram corrigidas (não são `@critical`, não bloqueiam o CI, mas estão partidas se alguém as correr)
