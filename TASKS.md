...
- [!] **NEX-178** — Corrigir concorrência do cliente em `/resumo` e adicionar E2E ao CI _(depende: NEX-065,NEX-175)_ — bug do cliente React e deadlock do Postgres já corrigidos e mesclados (#138, ver `docs/evidence/NEX-178_PUBLIC_BOOKING_CLIENT_CONCURRENCY.md`). Job `e2e-critical` implementado em `task/NEX-178-e2e-critical-ci` (PR #139 aberto, por mesclar): `.github/workflows/ci.yml` ganhou o job (build de produção + `supabase start` + `playwright test --grep @critical`), 6 specs marcadas `@critical`. Ao validar, `appointment-completion.spec.ts` (NEX-110) estava genuinamente partido (não ambiente) — o redesign da agenda mudou `.appointment-card` para `.appointment-timeline-card` e moveu o formulário de conclusão para um bottom sheet separado (`CompletionSheet.tsx`), e a spec nunca foi atualizada porque E2E nunca corria em CI; corrigido nesta tarefa. **Suite `@critical` completa (14 testes) validada e verde** contra o Supabase de dev/preview real, em paralelo e em modo fiel ao CI (1 worker, retries). Ainda bloqueada até: (1) a própria execução do job correr verde no GitHub Actions contra Postgres local via `supabase start` (Docker só disponível no runner, não validável nesta máquina); (2) a proteção de branch do `main` passar a exigir `e2e-critical` (configuração do GitHub, feita pela dona); (3) falta spec E2E completa de NEX-085 (criação manual de marcação) — só existe uma spec parcial (NEX-092); (4) `appointment-completion-discount.spec.ts`, `appointment-completion-extras.spec.ts` e `appointment-card.spec.ts` têm o mesmo padrão desatualizado do NEX-110 mas não foram corrigidas (não são `@critical`, não bloqueiam o CI, mas estão partidas se alguém as correr)
