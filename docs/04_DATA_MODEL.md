...
- `client_photos.storage_path` (`NEX-094`, `0019_client_photos_storage.sql`): a tabela e a sua RLS tenant-scoped já existiam desde `0001_initial.sql`, mas só nesta tarefa ganharam um bucket Storage privado (`client-photos`) por trás. Convenção de caminho `{tenant_id}/{client_id}/{photo_id}.jpg`; RLS de `storage.objects` compara só o primeiro segmento (`tenant_id`) com `current_tenant_id()` — o segundo segmento (`client_id`) não precisa de policy própria porque já está coberto pela RLS/FK composta da própria linha `client_photos` que sempre acompanha o objeto. Toda foto é reencodada para JPEG no upload (`src/lib/image-processing.ts`, via `sharp`), o que remove EXIF (incluindo geolocalização) por omissão e serve como verificação de assinatura — só bytes que decodificam como imagem real chegam ao Storage, independentemente do `Content-Type` declarado pelo browser.
- `appointments.booking_lookup_code_hash` (`0018_booking_lookup_code.sql`): alternativa ao `booking_token` de 64 hex para uma cliente que perdeu o link mas se lembra de um código curto — 8 caracteres alfanuméricos de um alfabeto de 32 símbolos (`23456789ABCDEFGHJKMNPQRSTUVWXYZ`, exclui `0/O/1/I/L` por ambiguidade visual/oral), ~1e12 combinações, a mesma margem de segurança do próprio token, deliberadamente maior que os 6 caracteres considerados inicialmente (ver commit) porque um código de 6 caracteres sozinho (sem telefone/e-mail emparelhado) é varrível por força bruta mesmo com rate limit. Mesma disciplina de `booking_token_hash`: só o hash SHA-256 é persistido, o código em texto claro só sai uma vez (retorno de `create_public_booking`, e-mail de confirmação) e nunca é recuperável depois — perdê-lo significa voltar ao link original. `resolve_booking_lookup_code(p_code)` devolve o detalhe completo da marcação diretamente (não um token) porque o token em texto claro nunca é armazenado e por isso não pode ser re-derivado a partir do código; resposta uniformemente vazia tanto para código malformado quanto para código desconhecido (mesma disciplina anti-enumeração de `resolveBookingByToken`, `docs/05_SECURITY_PRIVACY.md` T3).
- `business_settings.specialty`/`about_description`/`instagram_handle`/`logo_path`/`cover_image_path`/`booking_enabled` (visual refinement, `0030_business_public_profile.sql`): campos reais e opcionais que alimentam a página pública inicial (`/b/[slug]`) — especialidade, descrição "sobre", Instagram e logótipo/capa da marca, todos configuráveis em Definições, todos ocultos na página pública quando `null` (nunca placeholder). `instagram_handle` guarda só o identificador (sem `@`), validado por regex (`^[A-Za-z0-9._]{1,30}$`); a UI antepõe o `@` na apresentação. `logo_path`/`cover_image_path` seguem a mesma convenção de `client_photos`/`services.photo_path`: caminho de Storage, não URL — mas ao contrário de `client-photos` (privado), `business-logos`/`business-covers` são buckets **públicos** (`public=true`), porque a página que os mostra não tem sessão; a política de `select` em `storage.objects` existe só para acesso direto à API/tabela, a tag `<img>` da página pública lê pelo endpoint público sem passar pela RLS. `booking_enabled` (default `true`) permite à dona fechar temporariamente a marcação online sem despublicar a página — quando `false`, a página mantém morada/contacto/horário visíveis e troca o botão principal por "Contactar profissional". A mesma migração adiciona `get_public_business_hours(p_tenant_id)`, uma função `security definer` estreita que devolve só `day_of_week`/`is_open`/`opens_at`/`closes_at` — `business_hours` continua sem política `anon` própria (ver nota acima), mas a regra "agenda em bruto nunca exposta" é sobre não deixar um caller anónimo consultar internamente a tabela à vontade, não sobre esconder o horário de funcionamento em si (informação tão pública quanto uma ficha do Google Maps); mesmo padrão de superfície mínima do motor de disponibilidade. Concedida a `anon` **e** `authenticated` (ao contrário de `resolve_booking_lookup_code`, só `anon`) porque "Ver como a cliente vê" (Definições) abre esta mesma página numa nova aba que ainda carrega a sessão da dona — o pedido chega como `authenticated`, não `anon` — e o horário é igualmente público para qualquer chamador.
