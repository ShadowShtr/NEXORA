...
const header = [
  ...
  'responsabilidade',
  'camada',
  'importadores_principais',
  'dependencias_pesadas',
  'leitura_dados',
  'escrita_dados',
  'cache_atual',
  'invalidacao_atual',
...
for (const f of files) {
  ...
  const leitura = content
    ? content.match(/\.rpc\(/)
      ? 'rpc'
      : /\.from\(|\.select\(/.test(content)
        ? 'select/from'
        : ''
    : '';
  const escrita =
    content && /\.insert\(|\.update\(|\.upsert\(|\.delete\(/.test(content)
      ? 'mutacao-supabase'
      : '';
  const cache = content
    ? /no-store/.test(content)
      ? 'no-store'
      : /force-dynamic/.test(content)
        ? 'force-dynamic'
        : /unstable_cache/.test(content)
          ? 'unstable_cache'
          : /\bcache\(/.test(content)
            ? 'react-cache'
            : ''
  ...
  const invalidacao = content
    ? /revalidatePath/.test(content)
      ? 'revalidatePath'
      : /revalidateTag/.test(content)
        ? 'revalidateTag'
        : ''
    : '';
  ...
  let riscoSeg = '';
  if (content) {
    if (/service_role|SUPABASE_SERVICE_ROLE/.test(content) && camada === 'client')
      riscoSeg = 'ALTO: possivel service role em client';
    else if (/createAdminClient/.test(content) && camada === 'client')
      riscoSeg = 'ALTO: admin client referenciado em client';
    else if (/select\(['"]\*['"]\)/.test(content)) riscoSeg = "MEDIO: select('*') presente";
    else riscoSeg = 'sem sinal automatico';
  }

  const heavyDeps = content
    ? (content.match(/from\s+["'](pdfkit|exceljs|sharp|qrcode)["']/g) || []).join(';')
    : '';
