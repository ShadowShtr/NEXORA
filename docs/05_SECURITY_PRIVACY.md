...
| T8  | Logs com PII                              | vazamento secundário                    | `src/lib/logger.ts` (`NEX-170`) — todo campo de log passa por uma allowlist de tipos primitivos (sem objetos aninhados) mais redaction automática por nome de chave (`email`, `telefone`, `token`, etc.) e por forma do valor (e-mail/telemóvel E.164, mesmo sob uma chave inócua); correlation id por pedido (`src/middleware.ts` + `src/lib/request-id.ts`) para juntar linhas de log do mesmo pedido sem depender de PII para correlacionar                                                                                                                                                                                                             |
...
- log tests (redaction/allowlist) — `tests/unit/logger.test.ts` (`NEX-170`): redaction
  por nome de chave e por forma do valor, correlation id, e um teste de regressão
  específico para o erro `[object Object]` que apareceu ao logar um erro do Supabase
  sem esta proteção.
